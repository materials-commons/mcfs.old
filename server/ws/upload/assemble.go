...
func newAssembler(projectID, directoryID, fileID string) assembler {
	return assembler{
		...
		fileID:      fileID,
		log:         log.New("go-routine", "assembler"),
	}
}
...
func (a assembler) assembleFile() {
	...
	filePath := mc.FilePath(a.fileID)
	fdst, err := os.Create(filePath)
	if err != nil {
		a.log.Error(log.Msg("Error creating assembly file %s: %s", filePath, err))
		return
	}
	defer fdst.Close()
	...
	if err != nil {
		a.log.Error(log.Msg("Error reading assembly dir %s: %s", uploadPath, err))
		return
	}
	...
	for _, finfo := range finfos {
		...
		if err != nil {
			a.log.Error(log.Msg("Error reading chunk %s: %s", finfo.Name(), err))
			return
		}
		io.Copy(fdst, fsrc)
		fsrc.Close()
